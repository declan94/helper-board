...
- **18650 电池供电、深睡眠架构**:绝大部分时间深睡(屏幕靠 ST7305 低功耗模式自持画面),每 30 分钟醒来刷新温湿度/日期/默认餐次;缓存超过 2 小时或处于 6/10/15/20 点窗口时联网同步菜单(SNTP 校时 + 拉取今明两天记录,缓存进 NVS,断网时用缓存)
- **餐次自动切换**:05:00 早餐 → 10:00 午餐 → 15:30 晚餐 → 20:30 明日菜单预览
- **KEY 键**(GPIO18):单击循环切页(早/中/晚/明日),长按 ≥1 秒立即联网同步(改完表格想马上生效时用)
...
3. **KEY 单击**:1~2 秒内切到下一页;**长按 1 秒+**:联网同步(页脚"更新于"时间变化)
4. **改表格 → 长按 KEY**:屏幕内容更新
5. **餐次窗口**:各时段默认页正确;20:30 后显示"明日"
6. **断网降级**:关路由后长按 KEY,页脚显示"同步失败·缓存 xx-xx xx:xx"
