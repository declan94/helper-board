...
- **KEY 键**(GPIO18):短按循环切页(早/中/晚/明日),**按住约 1.5 秒**立即联网同步(改完表格想马上生效时用)。BOOT 键仅用于刷机,运行中勿按(strap 引脚,深睡中按住会假死,重新上电恢复)
...
3. **短按 KEY**:1~2 秒内切到下一页;**按住 KEY 约 1.5 秒**:联网同步(页脚 Updated 时间变化)
4. **改表格 → 长按 KEY**:屏幕内容更新
